...
for lig in lig_list:
    ...
    "METHOD Beta version of 2Vinardo, uses empirical docking scoring function F36 to predict ligand pose and binding affinity"
    ])
    #Define the path for the ligand
    lig_path1 = path + '/L1000_results/DOCK.F36.P2hvx/' + lig + '_ligand.sdf'
    lig_path2 = path + '/L1000_results/DOCK.F36.P1t31/' + lig + '_ligand.sdf'
    lig_path3 = path + '/L1000_results/DOCK.F36.P3n7o/' + lig + '_ligand.sdf'
    #Open the file
    ...
    # Define the paths
    protein_pdb_path = os.path.join("./L1000_results/Prot", f"{prot}_protein.pdb")
    ligand_info_path = os.path.join("./L1000_ligands", f"{lig}.tsv")
    ligand_pdbt_path = os.path.join(f"./L1000_results/DOCK.F36.P{prot}", f"{lig}_ligand.pdbt")
